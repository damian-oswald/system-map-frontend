...
| **Details** (`/entity/:id`)              | One page per element: a light grey banner with title, subclass tags, description, address, website, parent unit and figures aggregated down the hierarchy (sub-units, systems, data sets, services …); a _Kontext_ panel that embeds the map's network view around the element (1–3 steps, a click opens the neighbour's page); structure tree with a roll-up table; one small Oblique table per relation type; legal basis, keywords, architectures.                                                                                                                                                                                                                      |
