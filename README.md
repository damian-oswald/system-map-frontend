...
| **Map / Karte** (`/map`) | Two static layouts of the graph: *Layers* (organizations → IT systems → IT services → information units, crossing-minimized) and *Network* (force layout computed once in a web worker, so nothing wiggles). Per class (organizations, IT systems, IT services, information units) a level of detail – off, grouped to the top-level element, or detailed with the hierarchy drawn as an indented tree; filters for sector, relation types and subgraphs; search; focus on an element's 1–3-step neighbourhood; details drawer. Every setting is kept in the URL, so views can be shared. |
| **Inventory / Inventar** (`/catalog`) | Searchable, filterable inventory of information units, IT systems, IT services and organizations. Shows where data is stored and who is responsible. Group by system or organization, card or table view, CSV export. |
| **Details** (`/entity/:id`) | One page per element: description, relations diagram (incoming left, outgoing right, long groups expandable), organizational structure tree (ancestors, the element, all sub-units/parts) with a per-class roll-up (systems of sub-units, information units in sub-systems, storage of parts, users of sub-services), identifiers (Zefix, Staatskalender), legal basis, keywords |
| **About** (`/about`) | What the project is, how the app works, the SPARQL query, and a glossary generated from the ontology |

Every page ends with a dark **data section** (`app-data-footer`): name, creation and modification dates of the
dataset, source and triple count, named graph, SPARQL endpoint, the time of the last query and a reload button.
...
src/app/shared/
  ...
  ui.ts, bar-list.ts, tip.ts         entity chips, ranked bars, chart tooltip
src/app/features/
...
Colours: the four classes use a fixed palette (organization blue, IT system teal, information unit amber,
